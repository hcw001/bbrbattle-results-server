...
- [ ] **Survivors preserve damage state** — replace `toUnitCountMap` with a `SurvivingUnits` representation (`Array<{type, count, hpTaken}>`) so damaged capital ships are not collapsed into healthy ones; fix the outcome distribution hash key in `runSimulation` to include `hpTaken`, and update `BattleResponse`/`SimulateResponse` types accordingly. Found: `src/api/simulation.ts:10-16` and `src/api/schemas.ts:65-88` during API review — `toUnitCountMap` groups only by `unit.type`, silently discarding `hpTaken`; a damaged battleship and a healthy one serialize identically, which breaks IPC accounting and campaign survivor carryover.
- [ ] **Expose seed in API responses** — add `seed: number` to `BattleResponse` and `SimulateResponse` and return the seed used (auto-generated or caller-supplied) so any battle is fully replayable. Found: `src/api/simulation.ts:41` and `src/api/schemas.ts:67-88` during API review — the seed is computed locally in `runSingleBattle`/`runSimulation` but never included in the return value, making auto-seeded battles unreplayable.
- [ ] **Type `events` as `Event[]`** — change `BattleResponse.events` from `unknown[]` to the engine's `Event` discriminated union type. Found: `src/api/schemas.ts:73` during API review — `events: unknown[]` is a pure oversight; the fully-typed `Event` union exists in `src/engine/events.ts` and is already what the engine returns.
- [ ] **Target assignments as priority lists** — replace `subTargetAssignments`/`tacTargetAssignments` (`Record<string,string>` unit-ID maps) with `subTargets`/`tacTargets` (`UnitType[]` priority lists) in `SideInputSchema`; update `src/strategies/targetSelect.ts` to resolve a priority list to actual unit assignments at step 2. Found: `src/api/schemas.ts:41-49` during API review — the current interface requires callers to know internal unit IDs like `Submarine_0`, which are a leaky implementation detail of `buildBattleState`'s counter logic.
- [ ] **Shore bombardment schema** — add `bombardingUnits?: UnitInput` to `SideInputSchema` (attacker only) and `isAmphibious?: boolean` to `BattleRequestSchema`; wire both through `runSingleBattle` and `runSimulation` into `buildBattleState`. Found: `src/api/schemas.ts:35-50` during API review — `buildBattleState` in `src/engine/engine.ts:155` already accepts `bombardingUnits`, but `SideInputSchema` never exposes it, making shore bombardment inaccessible via the public API (only reachable through the legacy adapter).
- [ ] **Campaign endpoint** — add `POST /campaigns` route with a `CampaignRequest` / `CampaignResponse` schema and a thin adapter in `src/api/simulation.ts` that calls `runCampaign`; each battle's attacker units are overridden by survivors from the prior battle. Found: `src/api/routes.ts` and `src/engine/engine.ts:122-141` during API review — `runCampaign` is fully implemented in the engine but never exposed via HTTP. Requires: *Survivors preserve damage state*.
